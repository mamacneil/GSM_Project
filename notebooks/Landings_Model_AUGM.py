...
with pm.Model(coords=COORDS) as landings_model_x:
    ...
    plo_intercept = pm.Normal('plo_intercept', 0, 2)
    ...
    # Modify mixing
    #mixr = pm.HalfNormal("mixr", sigma=1.0)  # Learnable mixing parameter
    #mixr = pm.HalfNormal("mixr", sigma=1.0, dims=('exporter'))  # Learnable mixing parameter
    ...
    lsd_intercept_FAO = pm.Normal("lsd_intercept_FAO")
    lsd_intercept_sd_FAO = pm.Exponential("lsd_intercept_sd_FAO", 1)
    lsd_intercept_EX_FAO = pm.ZeroSumNormal("lsd_intercept_EX_FAO", sigma=lsd_intercept_sd_FAO, dims="exporter")    
    ...
    reliability_FAO = pyt.exp(
                    lsd_intercept_FAO + lsd_intercept_EX_FAO + lsd_unreliability_effect_FAO * unreliability_data
    )
...
with landings_model_x:
    #pm.sample(draws=1000, tune=2000, trace=ch_backend, idata_kwargs=dict(log_likelihood=False))
    idata_landings_x = pm.sample(draws=10, tune=10, idata_kwargs=dict(log_likelihood=False))
print('Done sampling')
...
# Export results
idata_landings_x.to_netcdf("idata-landings-model_AUGM_.nc")
print("Saved trace!")
